...
def get_ratio(token: str):
    ...
    response = requests.get(public_node_url + '/wasm/contracts/' + contract_address + '/store', params={'query_msg': query_msg}).json()
...
def get_balance(wallet: str, token: str):
    ...
    if token == 'bluna':
        ...
        response = requests.get(public_node_url + '/wasm/contracts/' + bluna_contract + '/store', params={'query_msg': query_msg})
