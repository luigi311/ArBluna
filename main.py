    ...
    while True:
        if ust_balance < min_ust_balance:
            ...
            if (luna_balance - amount) > min_trade_balance:
                ...
                notify('Not enough luna to sell while staying above the minimum trade balance')
                exit(1)
        ...
        while luna_balance > min_trade_balance:
            ...
            while min < buy_ratio:
                if flag_buy:
                    notify(f'Price is less than threshold\nMonitoring for price increase')
                    flag_buy = False
                price = get_ratio('bluna')
        ...
        while bluna_balance > min_trade_balance:
            ...
            while max > sell_ratio:
                if flag_sell:
                    notify(f'Price is greater than threshold\nMonitoring for price decrease')
                    flag_sell = False
                price = get_ratio('bluna')
                ...
                if price < max:
                    ...
                    if bluna_balance > min_trade_balance:
                        notify(f'Error: BLuna balance of {bluna_balance} should be low\nExiting')
                        exit(1)
